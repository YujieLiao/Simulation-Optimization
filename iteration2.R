...
while(cumul_value < 1000L) {
 ...
 # test <- cumsum(cumul_value_list [1:i 或者不写空着,1])
 #empty_data [i,"cumul_value"] <- test[i]
 # cumul_value <- sum(cumul_value_list[1:i,1])
...
## lapply l:list return a list of that stuff
...
lapply_example <- lapply(1:100, function(i) {
  ...
  return(new_data)  # or just type new_data
})

do.call("rbind", lapply_example)
#rbind is used to add rows together
#do.call can do a call together
...
map_df_example <- purrr::map_df(.x = 1:100, ~{
  ...
  new_data$trial <- .x #one thing different
